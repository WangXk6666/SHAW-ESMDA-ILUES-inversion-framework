...
The current workflow calls the modified Windows executable `SHAW303.EXE` included in this repository. Native Linux execution is not provided in this repository.
...
This repository includes a modified version of SHAW 3.0.3. The interactive operations in the original program were removed so that `SHAW303.EXE` can be called automatically during batch simulations. Keep the included executable and required SHAW input files in the location expected by `SHAW/SHAW_exe.py`. SHAW is not installed through `pip`.
...
This repository contains original integration, workflow, input/output processing, evaluation, and visualization code written by Xueke Wang. It also includes modified code based on the following third-party research products:
...
- **Modification in this repository:** The interactive operations in SHAW 3.0.3 were removed to support automated batch execution.
- **Requirement:** The original SHAW attribution, notices, and applicable terms are retained.

### 2. ESMDA

- **Original developers:** Alexandre A. Emerick and Albert C. Reynolds.
- **Original publication:** Emerick and Reynolds (2013).
- **Implementation in this repository:** Modified from code provided by the original authors.
- **Requirement:** The original attribution, copyright notices, and applicable license terms are retained.
...
The [MIT License](LICENSE) applies only to code for which Xueke Wang owns the copyright. It does not relicense the original SHAW code or the code adapted from the original ESMDA and ILUES implementations. All original copyright notices and applicable license terms are retained.
