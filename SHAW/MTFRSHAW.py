...
class MTFRSHAW:
    ...
    def _copy_essential_files(self, model_base_dir, thread_id):
        ...
        essential_files = ["INPUT.INP", "SHAW303.EXE", "SIT.INP", "WEATHER.INP", "TEMPERATURE.INP", "MOISTURE.INP"]
        for file in essential_files:
            src = os.path.join(model_base_dir, file)
            dst = os.path.join(thread_work_dir, file)
            if not os.path.exists(src):
                logging.warning(f"Source file does not exist, skipping copy: {src}")
                continue
            try:
                shutil.copy2(src, dst)
                if thread_id==1:
                    logging.info(f"Successfully copied file to thread {thread_id}: {file}")
            except Exception as e:
                logging.error(f"Failed to copy file (thread {thread_id}, file {file}): {str(e)}")
    ...
    def _worker(self, thread_id, param_thread, obs_type, start_index, relevant_headers, mapping, progress_bar=None):
        ...
        thread_exe_path = os.path.join(thread_work_dir,'SHAW303.EXE')
            ...
            for idx, params in enumerate(param_thread):
                ...
                while has_mistake and retry_count <= max_retry:
                    ...
                    if retry_count > 1:
                        ...
                        logging.info(f"Thread {thread_id}, Sample {idx}, Retry {retry_count}: "
                                     f"Sampling new parameters: {current_params}")
                    with self.dir_lock:
                    ...
                    try:
                        ...
                        if has_mistake:
                            ...
                            if retry_count >= max_retry:
                                logging.error(f"Thread {thread_id}, sample {idx}: Failed after {max_retry} retries;"
                                              f" marked as an execution error.")
                                with self.lock:
                                    self.error_count += 1
                                break
    ...
    def MTFRSHAW(self, N_threads, obs_type, relevant_headers, mapping, show_progress=True):
        ...
        logging.info(f'        Model computations for iteration {self.outer_iter} have been fully completed.       \n')
        logging.info(f'=============================================================================================')
